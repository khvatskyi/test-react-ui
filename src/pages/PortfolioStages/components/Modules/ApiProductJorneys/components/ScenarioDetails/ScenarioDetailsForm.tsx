...
export default function ScenarioDetailsForm({ stateCode }: IScenarioDetailsFormProps) {
  ...
  const onSave = async (details: IScenarioDetails) => {
    ...
    const formData: IScenarioDetails = {
      ...
      context: form.lens.prop('context').toProps().value ? form.lens.prop('context').toProps().value : '',
    }
  
    dispatch(setPending(true));
    try {
      /*const response =*/ await dispatch(initApiProductJourney(formData)).unwrap();
      const result = { form: formData } as FormSaveResponse<IScenarioDetails>;
